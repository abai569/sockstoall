...
// 初始化时从 localStorage 读取 token（页面刷新时）
...
export const xrayApi = {
  ...
  startNode: (nodeId: string) => api.post('/xray/start-node', { nodeId }),
  
  start: (routeId: string) => api.post('/xray/start', { routeId }),
  
  stop: (id: string) => api.post('/xray/stop', { id }),
  
  stopAll: () => api.post('/xray/stop-all'),
};
